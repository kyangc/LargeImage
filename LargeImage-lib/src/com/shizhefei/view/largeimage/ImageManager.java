...
public class ImageManager {
    ...
    public List<DrawData> getDrawData(float imageScale, Rect imageRect) {
        ...
        if (cacheImageData == null) {
            try {
                ...
                int s = (int) Math
                        .sqrt(imageWidth * imageHeight / (screenWidth / 2) / (screenHeight / 2));
        ...
        if (loadData.mCurrentCacheData != null && loadData.mCurrentCacheData.scale != scale) {
            cacheDatas.add(new CacheData(loadData.mCurrentCacheData.scale,
                    new HashMap<Position, Bitmap>(loadData.mCurrentCacheData.images)));
        ...
        if (loadData.mCurrentCacheData == null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (scale == cacheData.scale) {
                    loadData.mCurrentCacheData = new CacheData(scale,
                            new ConcurrentHashMap<Position, Bitmap>(cacheData.images));
                    iterator.remove();
                }
            }
        }
        if (loadData.mCurrentCacheData == null) {
            ...
            for (int row = startRow; row <= endRow; row++) {
                for (int col = startCol; col <= endCol; col++) {
                    ...
                    handler.sendMessage(
                            handler.obtainMessage(what, new MessageData(position, scale)));
                }
            }
        ...
        if (!needShowPositions.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (scaleKey / scale == 2) {// 这里都是大于scale的.,缩小的图，单位区域大
                    ...
                    Iterator<Entry<Position, Bitmap>> imageiterator = cacheData.images.entrySet()
                            .iterator();
                    while (imageiterator.hasNext()) {
                        Entry<Position, Bitmap> entry = imageiterator.next();
                        Position position = entry.getKey();
                        if (!(startRowKey <= position.row && position.row <= endRowKey
                                && startColKey <= position.col && position.col <= endColKey)) {
                            imageiterator.remove();
                        }
                    }

                    Iterator<Entry<Position, Bitmap>> imagesIterator = cacheData.images.entrySet()
                            .iterator();
                    while (imagesIterator.hasNext()) {
                        ...
                        Log.d("nnnn",
                                " bitmap.getWidth():" + bitmap.getWidth() + " imageHeight:" + iH);
                        for (int row = startPositionRow, i = 0; row <= endPositionRow; row++, i++) {
                            int top = i * blockImageSize;
                            if (top >= iH) {
                                break;
                            }
                            for (int col = startPositionCol, j = 0; col <= endPositionCol;
                                    col++, j++) {
                    ...
                    int startRowKey = imageRect.top / size + (imageRect.top % size == 0 ? 0 : 1)
                            - 1;
                    int endRowKey = imageRect.bottom / size + (imageRect.bottom % size == 0 ? 0
                            : 1);
                    int startColKey = imageRect.left / size + (imageRect.left % size == 0 ? 0 : 1)
                            - 1;
                    int endColKey = imageRect.right / size + (imageRect.right % size == 0 ? 0 : 1);

                    Log.d("nnnn",
                            "startRowKey" + startRowKey + " endRowKey:+" + endRowKey + " endColKey:"
                                    + endColKey);
                    ...
                    while (imageiterator.hasNext()) {
                        ...
                        if (!(startRowKey <= position.row && position.row <= endRowKey
                            ...
                            Bitmap bitmap = entry.getValue();
                            tempPosition.set(position.row / 2 + (position.row % 2 == 0 ? 0 : 1),
                                    position.col / 2 + (position.col % 2 == 0 ? 0 : 1));
    ...
    private static class InputStreamBitmapRegionDecoderFactory
            implements BitmapRegionDecoderFactory {
    ...
    private class LoadHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MESSAGE_LOAD) {
                if (loadData.mFactory != null) {
                    try {
                        ...
                        if (onImageLoadListenner != null) {
                            onImageLoadListenner.onImageLoadFinished(loadData.mImageWidth,
                                    loadData.mImageHeight);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                ...
                try {
                    loadData.mCacheImageData = loadData.mDecoder.decodeRegion(
                            new Rect(0, 0, loadData.mImageWidth, loadData.mImageHeight),
                            decodingOptions);
